...
//     let eval_lag_start = Instant::now();
//     let lag_evals_2k0 = (k..2 * k).map(|i| lag_bases_0.evaluate(i)).collect();
//     let lag_evals_2k1 = (k..2 * k).map(|i| lag_bases_1.evaluate(i)).collect();
//     let eval_lag_time = eval_lag_start.elapsed();
//     println!("Lagrange Evaluation time: {:?}", eval_lag_time);

//     let mut lag_ptr = &lag_bases_0;
//     let mut lag_evals_ptr = &lag_evals_2k0;
